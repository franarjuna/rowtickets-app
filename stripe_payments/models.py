...
class StripePaymentMethod(PaymentMethod):
    ...
    def create_checkout(self, order, root_url):
        ...
        url = 'https://api.stripe.com.ar/api/v2/orders'
    ...
    class Meta:
        verbose_name = _('Método de pago Stripe')
        verbose_name_plural = _('Métodos de pago Stripe')


class StripePayment(AbstractBaseModel):
    order = models.ForeignKey(
        'orders.Order', related_name='stripe_payments', verbose_name=_('compra'),
        blank=True, null=True, on_delete=models.PROTECT
    )
    payment_method = models.ForeignKey(
        StripePaymentMethod, verbose_name=_('medio de pago'), on_delete=models.PROTECT,
    ...
    class Meta:
        verbose_name = _('Pago Stripe')
        verbose_name_plural = _('Pagos Stripe')
        ordering = ('-created', )


class StripeIPN(AbstractBaseModel):
    payment = models.ForeignKey(
        StripePayment, verbose_name=_('Pago Stripe'), on_delete=models.PROTECT,related_name='ipns'
    )

    data = models.JSONField(verbose_name=_('data (request)'))

    class Meta:
        verbose_name = _('IPN Stripe')
        verbose_name_plural = _('IPNs Stripe')
        ordering = ('-created', )
