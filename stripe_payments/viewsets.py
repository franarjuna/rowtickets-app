...
from stripe_payments.models import StripeIPN, StripePayment, StripePaymentMethod
import requests
...
class StripeViewSet(viewsets.GenericViewSet):
    ...
    def create_checkout(self, request, *args, **kwargs):
        ...
        try:
            ...
            stripe = StripePaymentMethod.objects.get(
                country=self.kwargs['country_country'],
                identifier=serializer.validated_data['payment_method_identifier'],
                active=True
            )
            checkout_id = stripe.create_checkout(order, '')

            return Response({ 'checkout_id': checkout_id })
        except (Order.DoesNotExist, StripePaymentMethod.DoesNotExist):
            raise Http404
    ...
    def ipn(self, request, *args, **kwargs):
        ...
        payment = get_object_or_404(StripePayment, checkout_id=checkout_id)

        StripeIPN.objects.create(
            payment=payment,
            data=request.data
        )
